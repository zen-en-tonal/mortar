defmodule Mortar.Storage.LocalTest do
  ...
  describe "get/1" do
    ...
    test "returns {:ok, value} when key exists" do
      :ok = Local.put(@test_key, @test_value)
      assert {:ok, @test_value} = Local.get(@test_key)
    end

    test "returns binary data as-is" do
      binary_data = <<1, 2, 3, 4, 5>>
      :ok = Local.put(@test_key, binary_data)
      assert {:ok, ^binary_data} = Local.get(@test_key)
    end
  end

  describe "put/2" do
    test "stores key-value pair successfully" do
      assert :ok = Local.put(@test_key, @test_value)
      assert {:ok, @test_value} = Local.get(@test_key)
    end
    ...
    test "overwrites existing value" do
      :ok = Local.put(@test_key, "old value")
      :ok = Local.put(@test_key, "new value")
      assert {:ok, "new value"} = Local.get(@test_key)
    end

    test "handles binary data correctly" do
      binary_data = :crypto.strong_rand_bytes(1024)
      assert :ok = Local.put(@test_key, binary_data)
      assert {:ok, ^binary_data} = Local.get(@test_key)
    end
  end
  ...
  describe "delete/1" do
    ...
    test "only deletes the specific key" do
      ...
      :ok = Local.put(key1, "value1")
      :ok = Local.put(key2, "value2")

      assert :ok = Local.delete(key1)
      assert {:error, :not_found} = Local.get(key1)
      assert {:ok, "value2"} = Local.get(key2)
    end
  end
  ...
  describe "concurrent operations" do
    test "handles multiple put operations" do
      ...
      tasks =
        for key <- keys do
          Task.async(fn -> Local.put(key, "value_#{key}") end)
        end
      ...
      for key <- keys do
        expected = "value_#{key}"
        assert {:ok, ^expected} = Local.get(key)
      end
    end
  end
end
